...
def perform_subclustering_dendrogram(embeddings, cluster_labels, cluster_to_soap):
    """
    ...
    # Perform subclustering for each cluster
    for cluster, indices in cluster_indices.items():
        if len(indices) < 2:
            # Not enough samples for subclustering, assign to subcluster 0
        ...
        linkage_sub = linkage(cluster_embeddings, method='ward')
        ...
        for idx, sublabel in zip(indices, sub_labels):
            subcluster_labels[idx] = int(sublabel)
...
def perform_hierarchical_clustering(embeddings, n_clusters=4):
    ...
    if len(embeddings) == 0:
        raise ValueError("Cannot perform hierarchical clustering on empty embeddings array")
...
def visualize_clustering(linkage_matrix, cluster_labels, phrases, subcluster_labels, output_dir="visualizations"):
    """
    Visualize clustering results with cluster and subcluster labels inside the dendrogram.
    ...
    # Create mapping of cluster and subcluster to phrases
    cluster_subcluster_phrases = defaultdict(lambda: defaultdict(list))
    for i, (cluster, phrase) in enumerate(zip(cluster_labels, phrases)):
        subcluster = subcluster_labels[i]
        cluster_subcluster_phrases[cluster][subcluster].append(phrase)

    # Create leaf labels array with cluster-subcluster format
    leaf_labels = []
    for i in range(len(cluster_labels)):
        cluster = cluster_labels[i]
        subcluster = subcluster_labels[i]
        leaf_labels.append(f"{cluster}-{subcluster}")

    # Plot dendrogram with custom leaf labels and internal merge labels
    ...
    # Annotate merge points (connections) with merge index or distance
    icoord = dendro['icoord']
    dcoord = dendro['dcoord']
    for i, (xs, ys) in enumerate(zip(icoord, dcoord)):
        x = 0.5 * (xs[1] + xs[2])  # X coordinate of the merge
        y = ys[1]                 # Y coordinate (height) of the merge
        label = f"{y:.2f}"        # Use merge distance
        ax.text(x, y, label, va='bottom', ha='center', fontsize=8, color='red')

    ax.set_title("Hierarchical Clustering Dendrogram with Merge Labels")
    ax.set_xlabel("Sample")
    ax.set_ylabel("Distance")
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, "dendrogram_labeled.png"))
    plt.close()
...
def process_phrase_set(phrases, phrase_set_index, tokenizer, model, output_base_dir="results_ensemble"):
    ...
    if not phrases:
        print(f"Warning: No phrases provided for phrase set {phrase_set_index}")
        return None
    ...
    if not all_embeddings:
        print(f"Warning: No embeddings were generated for phrase set {phrase_set_index}")
        return None
        
    combined_embeddings = np.mean([emb for emb in all_embeddings.values()], axis=0)
    
    # Check if combined embeddings are empty
    if len(combined_embeddings) == 0:
        print(f"Warning: Combined embeddings are empty for phrase set {phrase_set_index}")
        return None
...
def main_hc():
    ...
    all_results = []
    for i, phrases in tqdm(enumerate(all_phrases[0:200]), desc="Processing phrase sets"):
