...
def map_clusters_to_soap(cluster_labels, embeddings, phrases):
    ...
    for label, centroid in cluster_centroids.items():
        sims = {k: cosine_similarity([centroid], [v])[0][0] for k, v in prompt_embeds.items()}
        prompt_assignments[label] = max(sims, key=sims.get)
    ...
    for label in unique_labels:
        cluster_phrases = [phrases[i] for i in range(len(phrases)) if cluster_labels[i] == label]
        if len(cluster_phrases) < 2:
            topic_assignments[label] = 'S'  # fallback
            continue
        ...
        # Match top words to SOAP keywords
        scores = {cat: sum(word in topic_keywords[cat] for word in top_words) for cat in topic_keywords}
        topic_assignments[label] = max(scores, key=scores.get)
